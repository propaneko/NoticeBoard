...
**Edit**, **delete**, **bump**, and **move** ink buttons appear only when the client thinks you may edit (same rules as the server, below). **Move** is hidden when Manual Pin is off or Legacy Board is on. A **map** ink button appears on every notice that has a location attached; any reader can click it to add a waypoint at those HUD X/Z (`y=0`) using the notice's **name**, **icon**, and **color** (default steelblue circle). Empty name uses the board name. `/tpwp` to that pin may land at world height 0. A **tablet** ink button appears on every row when [Scribe](https://mods.vintagestory.at/scribe) is loaded; any reader can click it. The first line becomes a checkbox task; the rest is a nested note. Hover-only extraInfo is the board name, author (Someone if anonymous), and the in-game date. That needs a writeable Scribe item (last opened, else first); Scribe explains if none. The notice stays on the board.
