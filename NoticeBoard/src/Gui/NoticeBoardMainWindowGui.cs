...
public class NoticeBoardMainWindowGui : GuiDialog
{
    ...
    public override void OnGuiOpened()
    {
        ...
        var scrollableContent = GuiComposerHelpers.AddVerticalScrollbar(dialogComposer, (float value) => {
            OnNewScrollbarValue(value);
        ...
        if (this.messages != null && this.messages.Count > 0)
        {
            for (int i = 0; i < this.messages.Count; i++)
            {
                ...
                double textHeight = this.CalculateTextHeight(message, (double)(insetWidth));
                ...
                bool canEdit = !this.isLocked || this.boardPlayerId == capi.World.Player.PlayerUID;
                if (canEdit)
                {
                ...
                totalContentHeight += rowHeight + 32.0f;
        ...
        {
            scrollArea.Add(new GuiElementStaticText(this.capi, "", (EnumTextOrientation)0, containerRowBounds, CairoFont.WhiteDetailText()), -1);
            totalContentHeight = minRowHeight + 60.0;
        }

        base.SingleComposer.Compose(true);

        float scrollVisibleHeight = (float)insetBounds.InnerHeight;
