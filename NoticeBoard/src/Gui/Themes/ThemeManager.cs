...
public static class ThemeManager
{
    ...
    public static string Sanitize(string themeName)
    {
        if (string.IsNullOrEmpty(themeName))
            return "";
        return themesList.Exists(theme => theme.Name == themeName) ? themeName : "";
    }

    public static ParchmentPalette Resolve(string messageTheme, string boardTheme)
    {
        return GetCurrentTheme(Sanitize(messageTheme) == "" ? boardTheme : messageTheme);
    }

    public static ParchmentPalette Age(ParchmentPalette src, double age01)
    {
        if (src == null || age01 <= 0)
            return src;

        double t = Math.Clamp(age01, 0, 1);
        return new ParchmentPalette(
            src.Name,
            Lerp4(src.InkColor, new[] { 0.16, 0.10, 0.05, 1.0 }, t),
            src.LinkColor,
            Lerp3(src.BaseRGB, new[] { 0.50, 0.38, 0.18 }, t),
            Lerp3(src.DarkRGB, new[] { 0.28, 0.18, 0.08 }, t),
            Lerp3(src.LightRGB, new[] { 0.62, 0.50, 0.28 }, t)
        );
    }
